from flask import Blueprint, render_template, request, flash, redirect, url_for, json, jsonify, Flask
from .models import User, User_role_change_request, Equipment, Image, Request, Notification
...
app = create_app()
@auth.route('update-requests')
def update_requests():
    threading.Timer(10, update_requests).start()
    with app.app_context():
        current_date = datetime.utcnow().strftime('%Y-%m-%d')
        current_date = datetime.strptime(current_date, '%Y-%m-%d').date()
        requests_to_update = Request.query.filter(Request.end_date < current_date).all()
        
        for request in requests_to_update:
            if request.status == 5:
                request.status = 7
                db.session.commit()
            elif request.status == 1 or request.status == 4:
                request.status = 6
                db.session.commit()
        
        
        requests_to_update = Request.query.filter(Request.start_date <= current_date,
                                                  Request.status == 2).all()
        for request in requests_to_update:
            request.status = 4
            db.session.commit()
        
        print("Requests have been updated!")
...
def send_noti(recipient, title, content, href):

    notidate = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    notidate = datetime.strptime(notidate, '%Y-%m-%d %H:%M:%S')
        
    noti = Notification(user_id = recipient, noti_title = title, noti_content = content, noti_href = href,
                        noti_date = notidate)
    db.session.add(noti)
    db.session.commit()

@auth.route('/notifications/<int:id>/read', methods=['POST'])
@login_required
...
def base():
    ...
    if current_user.is_authenticated:
        ...
        unread = Notification.query.filter(Notification.user_id == current_user.id, Notification.unread == 1).count()
    ...
    if (request.method == 'POST'):
        ...
        for change in changes:
            ...
            if (change[0] == 'change category'):
                ...
                admins = User.query.filter(User.access >= 3).all()
                for admin in admins:
                    send_noti(admin.id, "New request", username + " has sent a new role change request", '/admin/user-role-change-request-list')
...
def borrowing_request_list():
    ...
    search_by_username = request.args.get('search')
    
    if search_by_username:
        tmp = []
        for request_ in request_list:
            if search_by_username.lower() in request_.user_name.lower():
                tmp.append(request_)
        request_list = tmp
    else:
        search_by_username = ''
    ...
    if not search_by_status:
        search_by_status = 'Pending'
...
@auth.route('/view-request-form/<int:id>', methods=['GET', 'POST'])
def view_request_form(id):
    request_ = Request.query.filter_by(id=id).first_or_404()
    return render_template('view_request_form.html', request=request_)
...
@auth.route('/admin/user-role-change-request-list/reject/<int:id>', methods=['POST'])
@login_required
...
@auth.route('/admin/user-role-change-request-list/accept/<int:id>', methods=['POST'])
@login_required
def accept_user_role_change_request(id):
    ...
    if request.method == 'POST':
        ...
        user.access = user_request.new_role
        db.session.commit()
        ...
        send_noti(current_user.id, "Announcement",'Your role change reqeust hass been approved! Your new role is: ' + ROLE[new_role], '/profile/'+current_user.user_name)
        flash('Request has been approved!', category='info')
        return redirect(url_for('auth.user_role_change_request_list'))
...
@auth.route('/equipments/overview', methods=['GET', 'POST'])
def equipments_overview():
    equipments = list(Equipment.query.order_by(Equipment.status))
    list_of_types = set()
    ...
    for type in list_of_types:
        ...
        rows.append(new_row)
    return render_template('equipments_overview.html', rows = rows)
...
def equipment_details(equip_name):
    ...
    requests = Request.query.filter(Request.equip_name==equip.name, 
                                    Request.status == 1).all()
    requests = list(requests)
    in_queue = len(requests)
   
    return render_template('equipment_details.html', equip=equip, images = images, len_image = len_image, requests = requests, in_queue = in_queue)
...
def send_request(user_name, equip_name):
    if request.method == 'POST':
        ...
        admins = User.query.filter(User.access >= 3).all()
        for admin in admins:
            send_noti(admin.id, "New request", user_name + " has sent a new borrowing request!", "/view-request-form/"+str(new_request.id))
        
        return redirect(request.referrer)
    


@auth.route('/admin/borrowing-request-list/approve-request/<int:id>', methods=['GET', 'POST'])
@login_required
def accept_borrowing_request(id):
    ...
    if request.method == 'POST':
        ...
        if equip.status != 1 or request_.status != 1: 
            flash('Cannot approve this request', category='error')
            return redirect(request.referrer)
        if request_.start_date <= current_date:
            request_.status = 4
        else:
            request_.status = 2
        equip.status = 3
        ...
        user = User.query.filter_by(user_name = request_.user_name).first_or_404()
        send_noti(user.id, "Announcement" ,"Your borrowing request has been apporved", "/view-request-form/"+str(request_.id))
        return redirect(request.referrer)
...
def reject_borrowing_request(id):
    ...
    if request.method == 'POST':
        ...
        if request_.status != 1:
            flash('Cannot reject this request', category='error')
            return redirect(request.referrer)
        ...
        user = User.query.filter_by(user_name = request_.user_name).first_or_404()
        send_noti(user.id, "Announcement" ,"Your borrowing request has been rejected", "/view-request-form/"+str(request_.id))
